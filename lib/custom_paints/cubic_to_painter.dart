...
class CubicToPainter extends CustomPainter {
  ...
  void cubicTo(Canvas canvas, Size size) {

    final path = Path()
      ..moveTo(point0.dx, point0.dy)
      ..cubicTo(point1.dx, point1.dy, point2.dx, point2.dy, point3.dx, point3.dy);
    ...
    ///p2
    drawDashLine(canvas, Offset(point3.dx, point3.dy), point2);
    drawText(canvas: canvas, position: Offset(point2.dx + 10, point2.dy - 10), text: point2.uiString("p2"), fontSize: 22);
    ///p3
    drawText(
        canvas: canvas, position: Offset(point3.dx - 120, point3.dy + 10), text: point3.uiString("p3"), fontSize: 22);
  }
...
class CubicWidget extends StatefulWidget {
  const CubicWidget({super.key});

  @override
  State<CubicWidget> createState() => _RelativeCubitWidgetState();
}

class _RelativeCubitWidgetState extends State<CubicWidget> {
        ...
        builder: (context, constraints) {
          return Column(
            children: [
              ...
              Text("void cubicTo(   double x1,   double y1,   double x2,   double y2,   double x3,   double y3, )", style: Theme.of(context).textTheme.titleMedium,),
              const SizedBox(height: 20,),
              FittedBox(
                child: CustomPaint(
                    size: size,
